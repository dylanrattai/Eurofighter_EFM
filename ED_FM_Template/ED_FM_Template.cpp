...
static Flight_Control_System* s_fcs = NULL;
static Electricsystem* s_electric = NULL;
...
void init()
{
	...
	s_fcs = new Flight_Control_System(*s_state, *s_input, *s_airframe);
	s_flightModel = new FlightModel(*s_state, *s_input, *s_engine, *s_airframe, *s_fuelsystem, *s_fcs);
	s_electric = new Electricsystem(*s_state, *s_input, *s_engine);
	...
	float value)
{
	...
	case COMMAND_THROTTLE:
		s_input->throttle2(value); //Had to swap these, easier than swapping in the engine code
		break;
...
void ed_fm_set_draw_args (EdDrawArgument * drawargs,size_t size)
{
	...
	drawargs[1015].f = s_fcs->getroll();//left Aileron
	drawargs[1016].f = -s_fcs->getroll();//right aileron
	drawargs[1011].f = s_fcs->getpitch(); //right elevator ist standard für ein Leitwerk
	drawargs[1012].f = s_fcs->getpitch();//left elevator
	drawargs[19].f = s_fcs->get_canard_anim(); //AOA based canard movement
	...
	drawargs[21].f = s_airframe->getSpeedBrakePosition();//airbrake #1
	...
	drawargs[28].f = s_engine->updateBurner();//Burner Stage 1 and 0 for left engine
	drawargs[29].f = s_engine->updateBurner2();//Burner Stage 1 and 0 for right engine
	...
	drawargs[1053].f = s_airframe->getLEFlapPosition();//Slats/LE-Flaps Position due to auto drive flaps left
	drawargs[1054].f = s_airframe->getLEFlapPosition();//Slats/LE-Flaps Position due to auto drive flaps right
	drawargs[22].f = s_airframe->getRefuelingDoor();//Refueling-Door toggle Function (Switched to probe)
	...
	drawargs[2005].f = s_electric->getPP5_Bus();
...
switch (index)
{
case ED_FM_OXYGEN_SUPPLY: //Oxygen stuff
	return 101000.0;
...
case ED_FM_ENGINE_1_RPM: 
	return s_engine->getRPMNorm() * 777.0;//RPM in Rad/s
