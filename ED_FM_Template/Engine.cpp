...
void Engine::hotInit()
{
	...
	m_spoolHOldSpool = 0.68;
	m_spoolCOldSpool = 0.68;

	m_rpmNormal2 = 0.68;
	m_rpmPrevious2 = 0.68;
	//m_input.m_engine_start = 1.0;
	m_spoolHOldSpool2 = 0.68;
	m_spoolCOldSpool2 = 0.68;
}
...
//--------------Start of Engine starting up and spooling down//Noch drüber nachdenken----------------------------	
	...
	//-------------Reset update Spool Cold Engine 2----
...
double Engine::FuelFlowUpdate() 
{
	...
	else if ((corrThrottle >= 0.85) && (m_ignitors1 == true) && (m_hasFuel == true))
	{
...
double Engine::updateSpool()
{
	...
	m_spoolFactor = (DAT_EngSpool[indexInArray]) / 7.0;//Variale indexInArray ergibt diejenige Zahl die dem Index in dem Array entspricht// /5 hinzugefügt, für langsameres Ansprechen testweise/ jetzt auf 6.0 erhöht/ auf 4.0 verringert, da kleinere Turbinen bei der G91Y
...
double Engine::updateSpoolCold()
{
	double spoolColdEnd = 0.71;
		...
		m_spoolCFactor = (DAT_EngSpool[indexInArray]) / 12.0;//von /10 auf / 15/ auf 22 hochgesetzt für langsameren Cold-Start/ Auf 15.0 runter gesetzt, da die kleinen Triebwerke schneller Hochfahren sollten//Variable indexInArray ergibt diejenige Zahl die dem Index in dem Array entspricht
...
double Engine::updateSpoolHot()
{
	...
	m_spoolHFactor = (DAT_HtoCspool[indexInArray]) / 60;//Variale indexInArray ergibt diejenige Zahl die dem Index in dem Array entspricht
...
double Engine::updateBurner()
{
	...
	if ((m_newThrottle >= 0.85) && (m_ignitors1 == true) && (m_hasFuel == true))
	{
		m_burner = 1.0;
	}
	else
...
double Engine::FuelFlowUpdate2()
{
	...
	m_spoolFactorFuel3 = m_spoolFactorFuel2 * 10;
	m_newSpoolStepFuel2 = m_deltaSpoolFuel2 * m_spoolFactorFuel3;
	...
	else if ((corrThrottle >= 0.85) && (m_ignitors2 == true) && (m_hasFuel == true))
	{
...
double Engine::updateSpool2()
{
	...
	m_spoolFactor2 = (DAT_EngSpool[indexInArray]) / 5.0;//Variale indexInArray ergibt diejenige Zahl die dem Index in dem Array entspricht// /5 hinzugefügt, für langsameres Ansprechen testweise/ jetzt auf 6.0 erhöht
	m_newSpoolStep2 = m_deltaSpool2 * m_spoolFactor2;
	m_newThrottle2 = m_oldThrottle2 + m_newSpoolStep2; // reduced from 7 to 6 to see if this is the right one
...
double Engine::updateSpoolCold2()
{
	...
	m_spoolCFactor2 = (DAT_EngSpool[indexInArray]) / 15;//von /10 auf / 15 hochgesetzt für langsameren Cold-Start//Variable indexInArray ergibt diejenige Zahl die dem Index in dem Array entspricht
...
double Engine::updateBurner2()
{
	...
	if ((m_newThrottle2 >= 0.85) && (m_ignitors2 == true) && (m_hasFuel == true))
	{
		m_burner2 = 1.0;
	}
	else
...
double Engine::tempInC2()
{
	...
	if (getRPMNorm2() > 0.0)
	{
		m_tempInC2 = (950 * getRPMNorm2()) * overHeatCount2();
	}
	else
	{
		m_tempInC2 = 0.0;
...
double Engine::overHeatCount2()
{
	...
	else if ((m_state.m_mach < 1.0) && (m_heatTimerUP2 > 1) && (m_heatFailure2 == false))
	{
		CounterBack = true;
	}
	...
	else if ((CounterBack == true) && (CounterGo == false) && (m_heatTimerUP2 > m_heatTimerDOWN2))
	{
		m_heatTimerDOWN2++;
	}

	m_heatTwo2 = (m_heatOne2 * (m_heatTimerUP2 - m_heatTimerDOWN2)) + 1.0;
...
double Engine::overHeat2()
{
	...
	else if (((m_heatFailure2 == true) ||(m_needRepair2 == true)) && (m_input.getEngineStart2() == 1.0))
	{
		//m_input.m_engine_start = 2.0;
		//m_input.m_engine_stop = 2.0;
		m_needRestart2 = true;
