...
(
	...
	//--------------ROLL----------------------------------------
	Clb(DAT_Clb, CON_Clb_min, CON_Clb_max),
...
void FlightModel::M_stab()
{
	...
	*/
	...
	m_moment.z += m_k * CON_mac * ((((1.35 * CmalphaNEW(m_state.m_mach)) * m_inducedPitZ) * m_corrAoA) + ((1.21 * -CmdeNEW(m_state.m_mach) *((((m_fcs.getpitch() * m_elevDeflection)) + m_input.getTrimmUp() - m_input.getTrimmDown() + m_airframe.getAutoPilotAltH()) * m_hStabDamage))) + (0.70 * CmFlap(m_state.m_mach) * m_airframe.getFlapsPosition()))
...
void FlightModel::N_stab()
{
	...
	*/
	...
	m_moment.y += m_q * (((3.1 - (2.55 * m_stallMult)) * -Cnb_b * m_corrBeta) + (((Cnda_b * (m_fcs.getroll() * m_ailDeflection)) * m_CnpStab)) + ((1.3 * -Cndr_b) * (-m_input.getYaw() * m_rudDeflection)))
		+ 0.25 * m_state.m_airDensity * m_scalarVelocity * CON_A * CON_b * CON_b * (((1.5 - m_stallMult) * (Cnp_b * m_state.m_omega.x)) + ((3.1 - (1.25 * m_stallMult)) * Cnr_b * m_state.m_omega.y) + ((1.95 - m_noVertTailEff) * m_CnStab));
}
...
void FlightModel::lift()
{
	...
	m_force.y += m_k * (((((CLa(m_state.m_mach) * m_corrAoA) * m_CLaCorrMulti) + ((0.65 * CLds(m_state.m_mach)) * (m_elevDeflection / 6)) + ((CLFlaps + CLblc) * m_flapDamage)) * ((m_lWingDamageCL + m_rWingDamageCL) / 2.0)) * m_zeroLift);
}
...
void FlightModel::drag()
{
	...
	m_force.x += -m_k * ((CDmin(m_state.m_mach)) + (1.02 * (CDa(m_state.m_mach) * abs(m_corrAoA))) + (CDeng(m_state.m_mach)) + CDGear + CDFlaps + CDBrk + CDBrkCht + CD_OverMach + m_cdminADD + CD_brFlap + m_stallIndDrag);
}
...
{	
	...
	m_force.z += m_k * ((Cydr(m_state.m_mach) * (m_fcs.getyaw() * m_rudDeflection)) + (Cyb(m_state.m_mach) * m_corrBeta)); //neu eingefügt 28Mar21
}
...
void FlightModel::thrustForce()
{
	...
	m_thrustForce = (((m_engine.updateThrust() / 1.35) + (m_engine.updateThrust2() / 1.35)) + m_addThrust) * m_airframe.getEngineDamageMult() * m_thinAirMulti;
...
void FlightModel::calcAeroDeflection()
{
	...
	{
		m_ailDeflection = 0.75 * (2.0 * CON_aitgu); // 2x Ausschlag da einmal hoch und einmal runter
	}
	if (m_airframe.getGearNPosition() == 0.0)     
	{
		m_ailDeflection = 0.75 * (2.0 * CON_aitnu);// 2x Ausschlag da einmal hoch und einmal runter
	}
	...
	if (m_fcs.getpitch() >= 0.0)
	{
		m_elevDeflection = m_fcs.getpitch() * CON_hstdUP;
	}
	if (m_fcs.getpitch() < 0.0)
	{
		m_elevDeflection = m_fcs.getpitch() * CON_hstdDN;
	}
...
void FlightModel::calcLiftFlaps()
{
	...
	else
	{
		...
		CLblc = (m_airframe.BLCsystem() * CLFlaps) * 0.8;//Doch BLC bei der F-4..hätt ich das gewusst :-)
...
void FlightModel::StabAugSystem()
{
	if ((m_airframe.getCompressorDamage() >= 0.35) && (m_input.getElectricSystem() == 1.0))
	{
		...
		m_CnStab = CnStab(m_state.m_mach) * m_state.m_omega.y;
		m_CnpStab = 0.15; //einfach als Multiplikator für Yaw-due-to-roll und yaw-due-to-aileron// war 0.25
		m_StabAugSys = 0.5;
	}
...
void FlightModel::calcZeroLift()
{
	...
	if (m_airframe.getWeightOnWheels() == 0.0)
	{
		...
		/*if ((AoAToWingRockQuotient >= 0.60) && (AoAToWingRockQuotient <= 1.5) && (m_wingStalling == false)) //das war zunächst 0.80
		{
			m_stallIndRock = 5.5 * (Clb_b * m_corrBeta); //war 3.5
		}
		else
		{
			m_stallIndRock = 0.0;
		...
		*/
		...
		{
			m_stallIndDrag = abs((CDa(m_state.m_mach) * abs(m_state.m_aoa)) * 0.5);//war 1.5 jetzt 2.2 jetzt 2.8 jetzt 3.0 jetzt 3.5 und 4.5 zu 3.5 zu 3.9
			m_noVertTailEff = 0.0; //Dieser Wert schaltet den Yaw-Stabilizer aus und sorgt für weniger YAW-Stabilisierung// was 1.9
		}
		else
		{
			m_stallIndDrag = 0.0;
		...
		if (m_airframe.getWeightOnWheels() == 0.0)
		{
			...
			{
				m_stallMult = 1.8 * FinalStallValue(AoAToStallAoAQuotient);
			}
			else
			{
				m_stallMult = 0.0;
		...
		//-----------------ALTE Version mit forced rolling--------------------------------------
		//---Stall-induced full-roll (flop-over-roll)------------------
		/*if ((m_state.m_aoa >= m_setLiftZero) && (m_state.m_angle.x >= 0.0) && ((CLaCLzeroQuotient > 1.5) && (m_state.m_aoa < 1.745)) && (m_inducedRollingRight == false) && (m_state.m_omega.x > -0.05)) // statt m_state.m_aoa jetzt CLaCLzeroQuotient
		{
			m_stallIndRoll = 0.020 * CLaCLzeroQuotient;  //0.25 jetzt mit weniger, weil ich den m_stallIndRoll vor die Klammer gezogen habe und er jetzt nicht mit kleiner 1 multipliziert wird.
								//von 0.25 auf 0.20 wegen Rotation der Momente wieder auf 0.30 wegen stärkerer Dämpfung zurück zu 0.15 zu 0.12 zu 0.09 zu 0.7 zu 0.3 zu 0.5
			if (m_state.m_omega.x >= 0.08)
			{
				m_inducedRollingRight = true;
			}
			else
			{
				m_inducedRollingRight = false;
		...
		else if ((m_state.m_aoa >= m_setLiftZero) && (m_state.m_angle.x < 0.0) && ((CLaCLzeroQuotient > 1.5) && (m_state.m_aoa < 1.745)) && (m_inducedRollingLeft == false) && (m_state.m_omega.x < 0.05)) // CLaQuot von 1.0 zu 1.2
		{
			m_stallIndRoll = -0.020 * CLaCLzeroQuotient;//-0.25 zurück zu 0.15

			if (m_state.m_omega.x <= -0.08)
			{
				m_inducedRollingLeft = true;
			}
			else
			{
				m_inducedRollingLeft = false;
...
void FlightModel::addedThrustCalc()
{
	...
	if ((m_engine.updateSpool() >= 0.85) && (m_engine.updateSpool2() >= 0.85))
	{
		m_addThrust = 0.5 * ((m_engine.updateSpool() * (AdThrLAlt(m_state.m_mach) * AdThrLAltMulti(m_airframe.getAltInFeet()))) + (m_engine.updateSpool2() * (AdThrLAlt(m_state.m_mach) * AdThrLAltMulti(m_airframe.getAltInFeet()))));
	}
	else if (((m_engine.updateSpool() < 0.85) && (m_engine.updateSpool2() >= 0.85)) || ((m_engine.updateSpool() >= 0.85) && (m_engine.updateSpool2() < 0.85)))
	{
		m_addThrust = 0.25 * ((m_engine.updateSpool() * (AdThrLAlt(m_state.m_mach) * AdThrLAltMulti(m_airframe.getAltInFeet()))) + (m_engine.updateSpool2() * (AdThrLAlt(m_state.m_mach) * AdThrLAltMulti(m_airframe.getAltInFeet()))));
	}
	else
	{
		m_addThrust = 0.0;
